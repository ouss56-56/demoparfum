...
import { createBrand, updateBrand, deleteBrand } from "@/app/[locale]/admin/actions/brand";
...
export default function BrandClientView({ brands }: { brands: BrandWithCount[] }) {
    ...
    const handleOpenModal = (brand: BrandWithCount | null = null) => {
                                ...
                                <tr key={brand.id} className="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className="px-2.5 py-1 bg-gray-100 rounded-lg text-xs font-medium text-gray-600">
                                            {brand._count.products} {t("table.products_count")}
                                        </span>
                                    </td>
                    ...
                    <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl relative">
                        <div className="px-6 py-4 flex items-center justify-between border-b border-gray-100 bg-gray-50/50">
                            <h2 className="text-lg font-bold text-primary-dark font-serif">
                                {editingBrand ? t("modal.edit") : t("modal.create")}
                            </h2>
                        ...
                        <form onSubmit={handleSubmit} className="p-6">
                            <div className="space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <input name="name" defaultValue={editingBrand?.name} required className="w-full bg-[#f8f9fa] border border-gray-200 text-sm rounded-xl focus:ring-2 focus:ring-[#D4AF37]/30 block px-4 py-3 outline-none" placeholder={t("modal.name_placeholder")} />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-xs font-semibold uppercase tracking-wider text-gray-500 ml-1">{t("modal.desc_label")}</label>
                                    <textarea name="description" defaultValue={editingBrand?.description || ""} rows={4} className="w-full bg-[#f8f9fa] border border-gray-200 text-sm rounded-xl focus:ring-2 focus:ring-[#D4AF37]/30 block px-4 py-3 outline-none" placeholder={t("modal.desc_placeholder")} />
                                </div>
                            </div>
